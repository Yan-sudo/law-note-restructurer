...
1. **Restructure notes** → 选课堂笔记（`.md`/`.docx`）并命名课程，插件构建整个知识库。
2. **阅读 + 批注**生成的页面；写在 **📝 My Notes** 区里的内容永久保留。
3. **之后加笔记** → **Update knowledge base**，自动只处理新增/改动并生成 **`What's New`** 变更图。
4. **Ask my notes** → 问答，或切换模式：**IRAC**（粘案情）、**练习**、**苏格拉底**、**中美对照**。
